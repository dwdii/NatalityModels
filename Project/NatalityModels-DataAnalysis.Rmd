...
The second multiple linear regression model uses predictor variables indicated as significant from the All Variables model. The adjusted \(R^2\) value for this model is `r round(smLmSigVars$adj.r.squared, 5)`.
...
colnames(cvLmResults) <- c("Model", "VS Error", "Adj R^2", "Variables", "VIF")
...
\[
  \begin{matrix}
      y_{targetAmt} = & `r lmSigLimVars$coefficients[1]` & `r lmSigLimVars$coefficients[2]` x_{sexF} & `r lmSigLimVars$coefficients[3]` x_{revokedYes}   \\
    & + `r lmSigLimVars$coefficients[4]` x_{isSportsCar} & &  
  \end{matrix}
\]

We can interpret the coefficients in the following manner. Holding all other predictors constant, for variable:

* *SEXz_F*, female drivers had a $ \(`r round(abs(lmSigCorVarsLM$coefficients[2]), 2)`\) decrease of claims on average compared to males.
* *REVOKEDYes*, drivers whose license had been revoked in the past 7 years had a $ \(`r round(abs(lmSigCorVarsLM$coefficients[3]), 2)`\) decrease in their claims on average compared to those who hadn't had their license revoked.
* *ISPORTSCAR*, drivers with a sports car had a $ \(`r round(abs(lmSigCorVarsLM$coefficients[4]), 2)`\) increase in their claims on average compared to those who didn't have a sports car.
* *BLUEBOOK*, a unit increase in the *BLUEBOOK* value of their car had a $ \(`r round(abs(lmSigCorVarsLM$coefficients[5]), 2)`\) increase in their claims on average.


Linear regression diagnostic plots are shown below. Residuals don't appear to be normally distributed and variance is difficult to judge. There are definitely some outliers which the model is not capturing.
...
Looking at the inverse response plot, there does not appear to be a pattern to the predicted response versus actual.
...
Running a more targeted auto-correlation analysis with R's *acf* function does not reveal any pattern of negative and positive residual oscilation.
