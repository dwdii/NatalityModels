...
We started with all possible predictor variables from our data sets in an ordinary least squares linear model. This model yielded a adjusted \(R^2\) of `r round(smLmAllVars$adj.r.squared, 5)`, but had significant variance inflation factor issues (> 120,000,000). 
...
A new model using guidance from the correlation analysis yielded our "High Correlation Variables" linear model. VIFs were better, but still well above our threshold of 10. Our fourth model was created using the R's *step* function. The "Step" model had an impressive adjusted \(R^2\) of `r round(smLmStep$adj.r.squared, 5)` for only 5 predictor variables. The variance inflation factors were improved, but unfortunately, still exceeded our threshold.

Next, we experimented with a revised "Significant Variables Minus" model which was based on the "Significant Variables" model but with three high VIF variables removed. 

## Model Validation

As mentioned earlier, a validation data set was reserved for use confirming the performance of each of the developed models. The results of the multiple linear regression model validation are shown below.
