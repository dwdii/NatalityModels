...
We used the R runtime via RStudio as our primary environment for all data steps including data preparation, exploration, analysis, model development, validation and selection. A GitHub repository was setup to facilitate collaboration amoungst the team, as well as to share our work in the spirit of reproducible research `r citep(bibGitHub)`.

## Data Preparation

Data sets from the Census Bureau, Centers for Disease Control, and Bureau of Labor Statistics were identified and downloaded to our project GitHub repository. These data sets were subsequently joined together in order to provide a unified data set for analysis and modeling. 
...
For the period of May 2010 - Decemeber 2015, the Census Bureau's census data was available as monthly population estimates broken down by age and gender `r citep(bibCensus2015)`. The age data was in whole year granulatity (i.e. 5 females age 10, 4 females age 11). We
created 3 buckets (10 years each) for the female population by age: 15-24, 25-34, and 35-44.
...
Again, running the auto-correlation analysis with R's *acf* function shows the same possible auto-correlation issue with lag 2 and 6.
...
The following plots show visually the performance of the significant models against the validation data set.
